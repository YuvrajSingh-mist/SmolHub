...
class COYODataset(Dataset):
    ...
    def __init__(self, 
        ...
        # Try to load COYO dataset with streaming, fallback to dummy if not available
        try:
            print("Attempting to load COYO-700M dataset with streaming...")
            dataset = load_dataset("kakaobrain/coyo-700m", 
                                 streaming=True,
                                 trust_remote_code=True)
            
            self.dataset_stream = iter(dataset[split])
    ...
    def _load_next_subset(self):
        ...
        try:
            for i, item in enumerate(self.dataset_stream):
                ...
                if (i + 1) % 1000 == 0:
                    print(f"Loaded {i+1}/{self.subset_size} samples...")
                    
            if len(new_data) < self.subset_size:
                print(f"⚠️  Stream exhausted! Only loaded {len(new_data)} samples.")
                self.exhausted = True
            else:
                print(f"Successfully loaded {len(new_data)} samples from COYO dataset")
...
train_dataset = COYODataset(
    split="train",
    subset_size=10000,  # Increased subset size
...
val_dataset = COYODataset(
    ...
    subset_size=1000,  # Increased validation set
    transform=test_tyransforms,
    tokenizer=tokenizer,
    max_length=ModelArgs.block_size
)

trainloader = DataLoader(train_dataset, batch_size=ModelArgs.batch_size, shuffle=True, num_workers=2)
valloader = DataLoader(val_dataset, batch_size=ModelArgs.batch_size, shuffle=False, num_workers=2)
...
if __name__ == "__main__":
    ...
    engine.initialize_wandb(
        ...
        config=config,
        run_name=f"siglip_lr{ModelArgs.lr}_bs{ModelArgs.batch_size}_epochs30"
    )
